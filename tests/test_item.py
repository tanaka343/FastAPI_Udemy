...
def test_find_by_name(client_fixture :TestClient):
    response = client_fixture.get("/items/?name=tanaka")
    assert response.status_code == 200
    item = response.json()
    assert len(item) == 1
    assert item[0]["name"] == "tanaka"

def test_create(client_fixture :TestClient):
    response = client_fixture.post("/items",json={"name":"suzuki","email":"hwou@com"})
    assert response.status_code == 201
    item = response.json()
    assert item["id"] == 3
    assert item["name"] == "suzuki"
    assert item["email"] == "hwou@com"

    response = client_fixture.get("/items")
    assert len(response.json()) == 3

def test_update_正常系(client_fixture :TestClient):
    response = client_fixture.put("/items/1",json={"name":"tanaka2"})
    assert response.status_code == 200
    item = response.json()
    assert item["name"] == "tanaka2"

def test_update_異常系(client_fixture :TestClient):
    response = client_fixture.put("/items/10",json={"name":"tanaka2"})
    assert response.status_code == 404
    assert response.json()["detail"] == "Item not found"
